...
const activeStage = computed(
  () => knowledgeStore.stages.find((s) => s.id === knowledgeStore.activeStageId) ?? knowledgeStore.stages[0]
)
...
<template>
  <div class="page-container" v-loading="knowledgeStore.loading">
    <PageHeader title="知识库" description="固定资产投资项目全生命周期管理流程标准内容（只读展示，由内容审核员统一维护）" />

    <el-alert type="info" :closable="false" style="margin-bottom: 16px">
      展示范围：项目阶段、关键环节/实施步骤、工作要求、法律政策制度依据、注意事项、潜在风险（不含「实施情况」）。
    </el-alert>
